...
const reminderSchema = new Schema(
  {
    ...
    id: {
      $type: String,
      validate: [v => validator.isUUID(v), 'Invalid uuid for task reminder.'],
      default: shared.uuid,
      required: true,
    },
    startDate: { $type: Date },
...
export const TaskSchema = new Schema(
  {
    ...
    alias: {
      ...
      match: [
        /^[a-zA-Z0-9-_]+$/,
        'Task short names can only contain alphanumeric characters, underscores and dashes.',
      ],
      validate: [
        {
          validator () {
        ...
        },
        {
          async validator (alias) {
            const taskDuplicated = await Task.findOne({
    ...
    userId: {
      $type: String,
      ref: 'User',
      validate: [v => validator.isUUID(v), 'Invalid uuid for task owner.'],
    }, // When not set it belongs to a challenge

    // Adventure/Project association
    adventureId: {
    ...
    challenge: {
      ...
      id: {
        ...
        ref: 'Challenge',
        validate: [
          v => validator.isUUID(v),
          'Invalid uuid for task challenge.',
        ],
      }, // When set (and userId not set) it's the original task
      taskId: {
        $type: String,
        ref: 'Task',
        validate: [
          v => validator.isUUID(v),
          'Invalid uuid for task challenge task.',
        ],
      }, // When not set but challenge.id defined it's the original task
      broken: {
        $type: String,
        enum: [
          'CHALLENGE_DELETED',
          ...
          'UNSUBSCRIBED',
          'CHALLENGE_CLOSED',
          'CHALLENGE_TASK_NOT_FOUND',
        ],
      }, // CHALLENGE_TASK_NOT_FOUND comes from v3 migration
    ...
    group: {
      id: {
        $type: String,
        ref: 'Group',
        validate: [v => validator.isUUID(v), 'Invalid uuid for group task.'],
      },
      ...
      assignedUsers: [
        {
          ...
          validate: [
            v => validator.isUUID(v),
            'Invalid uuid for group assigned user.',
          ],
        },
      ],
      assignedUsersDetail: {
      ...
      completedBy: {
        userId: {
          ...
          validate: [
            v => validator.isUUID(v),
            'Invalid uuid for task completing user.',
          ],
        },
        date: { $type: Date },
      },
...
TaskSchema.plugin(baseModel, {
  ...
  sanitizeTransform (taskObj) {
    if (taskObj.type && taskObj.type !== 'reward') {
      // value should be settable directly only for rewards
...
TaskSchema.statics.findMultipleByIdOrAlias = async function findMultipleByIdOrAlias (
  ...
  if (ids.length > 0 && aliases.length > 0) {
    query.userId = userId;
    query.$or = [{ _id: { $in: ids } }, { alias: { $in: aliases } }];
...
TaskSchema.statics.sanitizeUserGroupTask = function sanitizeUserGroupTask (
  taskObj,
) {
  const initialSanitization = this.sanitize(taskObj);
...
TaskSchema.statics.sanitizeChecklist = function sanitizeChecklist (
  checklistObj,
) {
...
TaskSchema.methods.scoreChallengeTask = async function scoreChallengeTask (
  delta,
  direction,
) {
  const chalTask = this;
  ...
  checklist: [
    {
      ...
      id: {
        ...
        validate: [
          v => validator.isUUID(v),
          'Invalid uuid for task checklist item.',
        ],
      },
      linkId: { $type: String },
    },
  ...
  _.defaults(
    {
      ...
      repeat: {
        // used only for 'weekly' frequency,
