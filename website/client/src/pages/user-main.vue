  ...
  >
    ...
    <template v-if="isUserLoaded">
      ...
      <div
        ...
        :class="{
          'no-margin': noMargin,
          'groups-background': $route.fullPath === '/group-plans',
        }"
      >
        <adventure-party-header
          v-if="
            isAdventureDetailsPage &&
            currentAdventure &&
            adventureCharacters.length > 0
          "
      ...
      <audio id="sound" ref="sound" autoplay="autoplay"></audio>
    </template>
  </div>
</template>
...
import AdventurePartyHeader from "@/components/header/adventurePartyHeader";
...
const bugReportSuccessModal = () =>
  import("@/components/bugReportSuccessModal");
...
export default {
  ...
  computed: {
    ...
    isAdventureDetailsPage() {
      const result = this.$route.name === 'adventureDetails' && this.$route.params.adventureId;
      console.log('isAdventureDetailsPage:', result, 'route name:', this.$route.name, 'adventureId:', this.$route.params.adventureId);
      return result;
    },
    currentAdventure() {
      const adventure = this.$store.state.currentAdventure;
      console.log('currentAdventure computed:', adventure);
      return adventure;
    },
    adventureCharacters() {
      const chars = this.$store.state.adventureCharacters || [];
      console.log('adventureCharacters computed:', chars, 'length:', chars.length);
      return chars;
    },
  },
  created() {
    ...
    Promise.all([
      ...
      .then(() => {
        ...
        if (analyticsConsent !== null) {
          ...
          if (analyticsConsent !== this.user.preferences.analyticsConsent) {
            this.$store.dispatch("user:set", {
              "preferences.analyticsConsent": analyticsConsent,
            });
          }
        }
        if (window && window["habitica-i18n"]) {
          if (
            ...
            window["habitica-i18n"].language.code
          ) {
            return null;
          }
        }
      ...
      .then(() => {
        ...
        let appState = getLocalSetting(
          CONSTANTS.savedAppStateValues.SAVED_APP_STATE
        );
        if (appState) {
      ...
      .catch((err) => {
        console.error(
          "Impossible to fetch user. Clean up localStorage and refresh.",
          err
